 ...
 */
/* PIAM Pro: cambiado de LV_STDLIB_BUILTIN a LV_STDLIB_CLIB.
 * Con PSRAM habilitada en el ESP32-S3, malloc()/free() reales acceden
 * a los 8MB de PSRAM automaticamente para asignaciones grandes, en vez
 * de quedar limitados al pool fijo de LV_MEM_SIZE (64KB) de abajo.
 * Necesario porque el proyecto usa una fuente custom grande (Bebas120,
 * ~868KB) y una imagen (~1.4MB). */
...
/** Enable log module */
/* PIAM Pro: activado (0 -> 1) para ver mensajes de LVGL en el Monitor
 * Serie durante el debug (asserts, mem, etc). Se puede volver a 0 mas
 * adelante si genera demasiado ruido en el log. */
#define LV_USE_LOG 1
#if LV_USE_LOG
    ...
    /* PIAM Pro: activado (0 -> 1) para que use printf (via Serial en Arduino-ESP32)
     * y los mensajes aparezcan directo en el Monitor Serie sin registrar un callback. */
